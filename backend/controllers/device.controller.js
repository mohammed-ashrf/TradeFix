...
exports.createDevice = async function (req, res) {
  try {
    const { clientName, telnum,deviceType,section,engineer,priority,clientSelection,complain,repair,notes,fees,finished, receivingDate, repaired, paidAdmissionFees, delivered, returned } = req.body;

    if (!clientName) {
      return res.status(400).json({ message: 'Name is required required' });
    }
    
    const device = new Device({ clientName, telnum,deviceType,section,engineer,priority,clientSelection,complain,repair,notes,fees,finished,receivingDate, repaired, paidAdmissionFees, delivered, returned});
